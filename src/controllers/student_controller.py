...
from common import Color, PasswordSecurer
...
class StudentController:
    ...
    def register_student(self, email: str, password: str, name: str) -> Student:
        ...
        new_student = Student(email, hashed_password, name)
    ...
    def login_student(self, email: str, password: str) -> Student:
        ...
        if student is not None and PasswordSecurer.verify_password(student.password, password):
            ...
            return student
        raise Exception("\tStudent does not exist. Check your email and password, or register if you haven't.")
    ...
    def verify_password(self, old_password: str) -> bool:
        ...
        if PasswordSecurer.verify_password(self.current_student.password, old_password): return True
    ...
    def change_student_password(self, new_password: str) -> None:
        ...
        print(Color.make_yellow('\tPassword changed successfully.'))
    ...
    def enroll_subject(self) -> None:
        ...
        if len(self.current_student.subjects) < 4:
            new_subject = Subject()
            print(Color.make_yellow(f'\t[ENROLLING] {new_subject}.'))
            
            if self.current_student.enroll_subject(new_subject):
                self.database.update_student(self.current_student)
                print(Color.make_yellow(
                    f'\tGPA recalculated: {self.current_student.average_mark:.2f}, '
                    f'Grade: {self.current_student.overall_grade} '
                    f'=> You are now enrolled in {len(self.current_student.subjects)}/4 subjects.'
                ))
            else: print(Color.make_red('\tYou have already enrolled in this subject.'))
        else: print(Color.make_red('\tStudents are allowed to enroll in 4 subjects only.'))
    ...
    def remove_subject_by_id(self, subject_id: str) -> None:
        ...
        if len(self.current_student.subjects) >= 0:
            print(Color.make_yellow(f'\t[REMOVING] Subject {subject_id}.'))
    ...
    def show_enrolled_subjects(self) -> None:
        ...
        print(Color.make_yellow(f'\tShowing {len(self.current_student.subjects)} enrolled subjects.'))
